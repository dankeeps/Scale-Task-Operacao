...
interface Props {
  ...
  onAdd: (name: string) => Promise<Avatar | null>;
}

// Select de Avatar (persona) com "+ adicionar" inline — espelha EditTypeSelect.
export function AvatarSelect({ items, value, onChange, onAdd }: Props) {
      ...
      {adding ? (
        <div className="flex gap-1.5">
          <Input
            ...
            placeholder="Novo avatar (persona)"
            disabled={saving}
            onKeyDown={(e) => {
        ...
        <div className="flex gap-1.5">
          ...
          <Button type="button" size="icon" variant="outline" className="shrink-0" onClick={() => setAdding(true)} title="Cadastrar avatar">
